...
def test_extractive_why_it_matters_for_top_story_only():
    text = BODY + " Higher interest rates could push mortgage costs to their highest level since 2008. Analysts cheered."
    ...
    assert top.why_it_matters in ("Higher interest rates could push mortgage costs to their highest level since 2008.",
                                  "Officials said the rate increase was needed because inflation remains high.")
...
def test_claude_why_it_matters_is_top_story_only_with_extractive_fallback():
    ...
    text = BODY + " Higher interest rates could push mortgage costs to their highest level since 2008."
    ...
    assert b.stories[0].why_it_matters and b.stories[0].why_it_matters != "ignored"  # extractive fallback
    assert b.stories[1].why_it_matters == ""


def test_extractive_why_skips_asides_and_tangents():
    from newsbrief.summarize import extractive_why

    text = ("Trump hosted Xi at a state dinner on Thursday. "
            "But one notable absence was Anthropic, one of the biggest AI firms in the world. "
            "The dinner could set the tone for trade talks between Trump and Xi next year.")
    s = Story([Article("Trump and Xi exchange warm words at state dinner", "u", "bbc", text=text)])
    s.summary = "Trump hosted Xi at a state dinner on Thursday."
    assert extractive_why(s) == "The dinner could set the tone for trade talks between Trump and Xi next year."
